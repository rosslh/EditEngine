...
class EditOrchestrator:
    ...
    def __init__(
        ...
        # API-level rate limiting: Semaphore controls concurrent LLM API requests
        # This provides defense-in-depth protection beyond Celery worker concurrency:
        # - Worker-level: Controls how many tasks can run simultaneously
        # - API-level: Controls how many LLM requests can be in-flight
        # Both limits are kept equal (DEFAULT_WORKER_CONCURRENCY) because each task
        # typically makes one LLM call, ensuring no bottlenecking at either level
    ...
    async def _process_edit_tasks(
        ...
        """Process a list of edit tasks with concurrency limits to prevent API rate limiting."""
        # Create a coroutine for each task with semaphore protection
        coroutines = [
            self._process_single_task_with_limit(task, paragraph_processor)
            for task in tasks
        ]
    ...
    async def _process_single_task_with_limit(
        self, task: EditTask, paragraph_processor: IParagraphProcessor
    ) -> EditResult:
        """Process a single edit task with semaphore protection for rate limiting."""
        async with self._api_semaphore:
            return await self._process_single_task(task, paragraph_processor)
